...
│
├── R/                                  # Internal helpers: sourced/rendered by the scripts above, never run directly (do not run or touch)
...
│
├── index.html                          # Coder-facing protocol app (copied into each batch folder). A coder should open this file to start coding papers from their batch.
│
├── all_articles_metadata.bib           # PROVIDE: BibTeX export (e.g. from Zotero) = input to script 2
├── all_articles.rds                    # PROVIDE: screening/inclusion decisions = input to script 2
│                                        #   (needs doi_no_slash + included columns)
├── Articles All Records/               # PROVIDE: the PDFs; script 1 writes the XML alongside them
...
└── Batches/                            # Generated by script 3: one folder + .zip per coder batch (gitignored)
    └── <coder_id>_<firstID>-<lastID>/
        ├── index.html                  #   the protocol app (opened by the coder to code the batch)
